...
fn std_load_i64(addr &i64, _ i64) {
	unsafe { _ = C.atomic_load_u64(voidptr(addr)) }
}
...
fn std_add_i64(addr &i64, delta i64) {
	unsafe { _ = C.atomic_fetch_add_u64(voidptr(addr), u64(delta)) }
}
...
fn std_swap_i64(addr &i64, val i64) {
	unsafe { _ = C.atomic_exchange_u64(voidptr(addr), u64(val)) }
}
...
fn std_load_i32(addr &i32, _ i32) {
	unsafe { _ = C.atomic_load_u32(voidptr(addr)) }
}
...
fn std_add_i32(addr &i32, delta i32) {
	unsafe { _ = C.atomic_fetch_add_u32(voidptr(addr), u32(delta)) }
}
...
fn std_swap_i32(addr &i32, val i32) {
	unsafe { _ = C.atomic_exchange_u32(voidptr(addr), u32(val)) }
}
